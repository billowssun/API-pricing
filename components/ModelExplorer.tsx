...
export function ModelExplorer({ models }: { models: Model[] }) {
  ...
  const [collapsed, setCollapsed] = useState<Set<string>>(
    () => new Set(providers.filter((p) => p.id !== "OpenAI").map((p) => p.id)),
  );
  const [expanded, setExpanded] = useState<string | null>(null);
  const groups = useMemo(
    () =>
      providers.flatMap((provider) => {
        if (selectedProvider !== "all" && provider.id !== selectedProvider)
          return [];
    ...
    [models, query, country, tier, sort, selectedProvider],
  );
  const count = groups.reduce((sum, g) => sum + g.rows.length, 0);
  const filtered = Boolean(
    query || country !== "all" || tier !== "all" || selectedProvider !== "all",
  );
  function reset() {
    setQuery("");
    setCountry("all");
  ...
  return (
    <div className="workbench-layout">
      <aside className="provider-index" aria-label="厂商索引">
        <div className="index-caption">PROVIDER INDEX</div>
        <button
          type="button"
        ...
        {providers
          .filter((p) => models.some((m) => m.provider === p.id))
          .map((p) => (
            <button
              type="button"
              key={p.id}
            ...
            >
              <ProviderMark provider={p.id} size="small" />
              <span>{p.name}</span>
            </button>
          ))}
      ...
      >
        <div className="board-heading">
          <span className="board-eyebrow">
            API PRICE INDEX / 主流模型价目簿
          </span>
          <h1>大模型价格，一目了然</h1>
          <p>
            快速查价、比较成本，找到适合你的模型。每百万 tokens · 原币报价。
          </p>
        </div>
        ...
        <div className="table-actions">
          ...
          <button
            type="button"
            disabled={filtered}
            onClick={() => setCollapsed(new Set(providers.map((p) => p.id)))}
          >
            收起全部
          </button>
        </div>
                  ...
                  >
                    <button onClick={() => toggleSort("input")} type="button">
                      ...
                      <IconArrowDown
                        size={13}
                        className={
                          sort === "input" ? "sort-active" : "sort-idle"
                        }
                      />
                    </button>
                  </th>
                            ...
                            >
                              <th scope="row">
                                ...
                                >
                                  ...
                                  {model.priceLabel ? (
                                    <small className="model-price-label">
                                      {model.priceLabel}
                                    </small>
                                  ) : null}
